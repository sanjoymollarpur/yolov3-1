    ...
    for thres in config.CONF_THRESHOLD:
        ...
        for lrate in config.LEARNING_RATE:
            ...
            train_loader, test_loader, train_eval_loader = get_loaders(
                train_csv_path=config.DATASET + "/neg-aug-test.csv", test_csv_path=config.DATASET + "/neg-aug-test.csv" #train-aug -> train, test-aug -> test
            )

            
            load_checkpoint(config.CHECKPOINT_FILE, model, optimizer, lrate)
            ...
            plt.ylabel('iou')
            plt.legend()
            plt.savefig(f"eval-graph/iou_lr-{lrate}_threshold-{thres}.png")
            plt.close()
            ...
            plt.ylabel('dsc, iou')
            plt.legend()
            plt.savefig(f"eval-graph/dsc_iou_lr-{lrate}_threshold-{thres}.png")
            plt.close()
            ...
            plt.ylabel('dsc')
            plt.legend()
            plt.savefig(f"eval-graph/dsc_lr-{lrate}_threshold-{thres}.png")
            plt.close()
            ...
            plt.ylabel('AP')
            plt.legend()
            plt.savefig(f"eval-graph/ap_lr-{lrate}_threshold-{thres}.png")
            plt.close()
        ...
        plt.legend()
        plt.savefig(f"eval-graph/ap_iou_f1_threshold-{thres}.png")
        plt.close()
        with open(f"metric/metric_for_cvc_threshold-{thres}.txt", 'w') as file:
