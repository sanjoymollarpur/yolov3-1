...
#python cml-inputvideo-completed.py polyp.mp4 p111.avi 10 .8
...
##################################################################
writer = imageio.get_writer(input_file, fps=frame_input)
###################################################################
...
if(len(sys.argv)<2):
    ...
    while success and vidcap.isOpened():
        ...
        if c%1==0:
            ...
            import time
            start=time.time()
            plot_couple_examples1(model, x, input_thresh, 0.5, scaled_anchors, c)
            # writer.append_data(image)
            # plt.show()
            end=time.time()
